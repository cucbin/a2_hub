...
function renderHighlightedFileLocation(resultsLoading, query, term) {
  ...
  return (
    <Fragment>
      ...
      {term.length === 0 ? null : (
        <span className="gitttree-search-term">{term}</span>
      )}
    </Fragment>
  );
}
...
function SearchBarResult({
  ...
  const resultClass =
    index === activeResult
      ? "gitttree-search-result gitttree-result-active"
      : "gitttree-search-result";
  ...
  return (
    ...
    >
      <div className="gitttree-search-file">
        <div className="gitttree-file-icon">
          <i className={fileIcons.getClassWithColor(fileName, false)}></i>
        </div>
        <div className="gitttree-search-filename">{fileName}</div>
      </div>
      <div className="gitttree-search-filelocation">
